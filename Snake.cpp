...
//获取缓冲区句柄
//“句柄”是Windows最常用的概念。它通常用来标识Windows资源
//(如菜单、 图标、窗口等)和设备等对象。虽然可以把句柄理解为是一个指针变量类型，
//但它不是对象所在的地址指针，而是作为Windows系统内部表的索引值来使用 的。调用相关文本界面控制的API函数。
...
//保存蛇的记录数据文件
static const char *Snake_Record[] = { "G://SnakeRecord//Snake1.txt",
"G://SnakeRecord//Snake2.txt",
"G://SnakeRecord//Snake3.txt",
"G://SnakeRecord//Snake4.txt",
"G://SnakeRecord//Snake5.txt" };

static const char *S_Armory[] = { "G://SnakeRecord//Armory1.txt",
"G://SnakeRecord//Armory2.txt",
"G://SnakeRecord//Armory3.txt" };

void Snake_class::JudgeChoice(COORD& cd , short incre)
{
	//判断指针指向哪个菜单选项
...
void Snake_class::StartGame(){
	...
	if (!pSnake->food_coord.X)
	{
		...
		while (1)
		{
			...
			//判断食物坐标是否与蛇身吻合
			vector<COORD>::iterator ite;
			for (ite = pSnake->Snake_coord.begin(); 
...
bool Snake_class::Read(size_t i)
{
	...
	for (size_t n = 1; n <= pSnake->Snake_size; ++n){
		inopen >> cd.X >> cd.Y;
...
{
	vector<COORD>::iterator hbeg = pSnake->Snake_coord.begin();
	vector<COORD>::iterator	beg = hbeg + 1;
...
void Snake_class::ShowSnake(COORD& cd)
{
	...
	vector<COORD>::reverse_iterator rbeg = pSnake->Snake_coord.rbegin();
	...
	for (vector<COORD>::iterator beg = pSnake->Snake_coord.begin();
